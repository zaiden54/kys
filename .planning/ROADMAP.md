...
- [x] 01-07-PLAN.md — Atomic single-statement upserts for salary, schedule, and YTD baseline, pinned by live concurrency race tests (CR-02 + WR-01 — SAL-02)
- [x] 01-08-PLAN.md — Residual review warnings: reconciling gross split, НДФЛ bracket-order assertion, statute-comment correction, DB money constraints, product metadata (WR-02..WR-05 — TAX-01, HOME-01)

**Gap wave 3** *(blocked on Gap wave 2)*

- [x] 01-09-PLAN.md — Reject sub-kopeck salary input gracefully and contain Server Action/client persistence failures (SAL-01)

**Gap closure round 2** *(added after the 2026-08-29 re-verification: 2/5 truths, 4 remaining BLOCKERs)*

**Gap wave 4** *(these two share no files and run in parallel)*

- [x] 01-10-PLAN.md — Accrue prior scheduled avans/salary payments into the cumulative НДФЛ base, replacing the constant-zero event sum (TAX-01, TAX-02, HOME-01, SAL-03)
- [x] 01-12-PLAN.md — Reject impossible calendar dates on both dated inputs and make the documented auth-secret template fail closed (SAL-01, SAL-02, SAL-03, AUTH-01)
...
**Goal:** As a signed-in user, I want to attach a one-off bonus or compensation (ex. sports) to a payment date, so that I can see how it affects my cumulative НДФЛ and future take-home payments.
**Mode:** mvp
**Depends on**: Phase 1
...
- [x] 02-01-PLAN.md — Tracer: bonus creation flows through the cumulative НДФЛ engine into the next-payment forecast (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 02-02-PLAN.md — Edit/delete a bonus (D-B04/D-B06) and the full UI-SPEC-compliant history list (wave 2)

**Gap closure** *(added after 02-VERIFICATION.md returned `gaps_found`: the MVP roadmap-contract pre-flight guard failed before BON-01/BON-02 were evaluated)*

**Gap wave 1**

- [x] 02-03-PLAN.md — Lock in the corrected user-story goal and close three evidenced code-review warnings (WR-01, WR-02, WR-03)

**Gap closure round 2** *(added after the 2026-08-30 re-verification: 12/14 must-haves verified, CR-01 blocking truths 11 & 12)*
...
- [ ] 04-01-PLAN.md — Annual summary engine + Recharts chart, wired end-to-end onto the home screen (HOME-02) (wave 1)
...
| 4. Annual Overview & PWA Installability | 0/2 | Not started | - |
